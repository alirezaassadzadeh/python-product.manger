...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(2,25,1350,750)
    ...
    def displayProducts(self):
        ...
        for row_data in query:
            ...
            for column_number, data in enumerate(row_data):
                if column_number==5 and data=='UnAvailable':
                    for i in range (1,5):
                        self.productsTable.item(row_number, i).setBackground(QColor (210,210,210))
...
class DisplayProduct(QWidget):
    ...
    def uploadImg(self):
        size =(256,256)
        self.filename,ok =QFileDialog.getOpenFileName(self,'Upload Image','','Image files (*.jpg *.png *.webp)')
        if ok:
